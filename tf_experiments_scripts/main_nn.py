...
def get_initilizations_HBF(init_args):
    if init_args.init_type == 'truncated_normal':
        inits_W = [None]
        inits_S = [None]
        inits_C = [None]
        ...
        indices = np.random.choice(X_train.shape[0],size=init_args.dims[1])
        #print np.transpose(init_args.X_train[indices,:]).shape
        inits_W.append(np.transpose(init_args.X_train[indices,:]) )
        for l in range(1,nb_hidden_layers):
            #inits_W.append( tf.truncated_normal(shape=[1,init_args.dims[l]], mean=init_args.mu[l], stddev=init_args.std[l], dtype=tf.float64) )
            inits_S.append( tf.constant(init_args.S_init[l], shape=[dims[l]], dtype=tf.float64 ) )
            #inits_C.append( tf.truncated_normal(shape=[init_args.dims[l],1], mean=init_args.mu[l], stddev=init_args.std[l], dtype=tf.float64) )
...
init_type = 'data_init'
init_args = ns.FrozenNamespace(init_type=init_type,dims=dims,mu=mu,std=std,b_init=b_init,S_init=S_init, X_train=X_train)
model = 'standard_nn'
model = 'summed_nn'
model = 'hbf'
#model = 'summed_hbf'
...
if model == 'standard_nn':
    ...
    (inits_C,inits_W,inits_S) = get_initilizations_HBF(init_args)
