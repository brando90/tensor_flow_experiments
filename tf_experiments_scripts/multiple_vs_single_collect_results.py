...
def process_file(dirpath,filename):
    ...
    raise('Unexepected error caused by %s'%filename)
...
for (dirpath, dirnames, filenames) in os.walk(path_to_experiments):
    ...
    if 'multiple' in dirpath or 'single' in dirpath:
        ...
        for filename  in filenames:
            (train_error, cv_error, test_error, results_current_mdl) = process_file(dirpath,filename)
            if cv_error < best_cv_errors:
                print 'relax'
                best_cv_errors = cv_error
                best_test_filname = filename
                results_best_mdl = results_current_mdl
        # now we have the best model (and hyperparams) for this experiment
        ...
        experiment_dir[nb_units] = {} # dict to collect data for best experiment
